...
**N is set by orthogonality**, not by speed. N is the number of genuinely disjoint slices the question decomposes into: distinct bodies of material, distinct search strategies. Fanning wider "for speed" spends the agent cap on redundant coverage and floods the hub with near-duplicate conclusions. N is capped by the 20-agent ceiling (the bound and its enforcement live in this skill's bounds conventions).
...
Both standing lenses are counted in the pre-flight estimate (roughly +2 at the floor), so "no surprise" holds even though they fit well under 20.
...
The fetch/verify agents already hold a source in context; **they** make a "do we trust this source" judgment there. **No new pass and no new lens**: the sniff test rides the agents already in context, so it stays free against the 20-agent cap.
...
- **Hard cap of 20 agents per research task.** The cap is hard, so the worst case has to fit, not just a typical run. This is an order of magnitude under the engine's ~97 for a full run, justified by orthogonal coverage rather than speed.
- **A token budget** on the run, held under the same no-silent-cap discipline as the agent count.

### The addable agent-count model

The shallow panel has its **own** agent-count model, and it is addable: a reader can name each component and sum them, and the worst case lands at or under 20.
...
The two fixed ends (scope + synth) cost **2**, leaving **18** for lenses plus verify vouchers. The bound that keeps the sum at or under 20 caps those two pools together: **N lenses at 6 or fewer** (the 2 standing lenses plus up to 4 disjoint orthogonality slices) and **12 or fewer verify vouchers** (single votes plus any generation-flagged second vouchers, counted against the same pool). Worst case: `1 + 6 + 12 + 1 = 20`, exactly at the ceiling. A decomposition that would push either pool past its share does not silently proceed; the pre-flight estimate below is where it surfaces.

This is **not** the engine's flat model. The engine spends `1 scope + 5 search + 15 fetch + (claims x 3 votes) + 1 synth`, whose fixed part alone (`1 + 5 + 15 + 1 = 22`) is already over 20 before a single verify vote, and its verify runs at roughly **77%** of the ~97 total. That model is kept here only as **context for what the shallow panel does not do**: the shallow panel has no separate `5 search + 15 fetch` fan-out to exempt, because **each lens does its own search, fetch, and distill inside one agent context**. The covering is the N lenses counted against the cap, not a breadth pool sitting outside it.
...
- the **pre-flight estimate** is the **before-the-run gate**,
- the **headline consumption** is the **after-the-run report**,
- the **running enforcement between them** is the **dispatching session's job**: it counts each Agent call it fires and does not cross 20.

This is what promptware can honestly claim (the dispatcher counts and stops), not a code guard. A firmer in-script guard is the same "enforce the bound in the script" effort the engine holdout defers; want it and that is a separate change against the workflow store, not a promise this skill makes.

### Pre-flight estimate

An **agent-count estimate surfaces before the run starts**, derived from the addable model above (scope + N lenses + verify vouchers + flagged seconds + synth) rather than asserted as a top-line number. It **auto-proceeds when under the cap**, and the run does **not** silently begin past an estimate that breaches it (a breaching estimate surfaces to the maintainer, who decides). The estimate **counts the two standing lenses** (prior-art / counterfactual and counter-evidence) in its N, roughly **+2 against the base the model expresses** (scope + the remaining orthogonality lenses + the per-claim vouchers + synth), so a reader can check that base + 2 lands under 20 rather than adding a delta to a floor the estimate never states.

### Headline consumption

Consumption is a **headline in every run's summary**, not a figure buried in a stats object: the **agent count and approximate tokens** report next to the findings. **Nearing or hitting the cap surfaces to the maintainer, never silently caps** the run. This extends the engine's own no-silent-cap discipline (it reports when a budget forces something to be dropped) to the verify phase, which the engine leaves unbounded.

A skill-only reader sees the shape the no-silent-cap claim rests on right here, so the pointer does not dangle:
...
Both lines belong to the run's full summary, whose complete shape (`Findings`, `Adversary verdicts`, `Evidence mode`, `Destination`, and these two consumption lines) is emitted by the `research-plan` orchestrator mode; this is the consumption slice of it, carried here so the headline and no-silent-cap claims resolve to a present artifact.
...
This is a named, **revival-triggered** follow-up, not a gap. **Revival trigger:** a literature lens is observed to overflow a clean context on a real research run (the shallow panel cannot hold one lens's reads before it distills). At that point the bounded deep-exploration Workflow lands here, with its bound enforced inside the Workflow (a shard cannot spawn un-counted sub-agents past the cap).
