...
- **Format cross-link pointers as prose paragraphs, not bulleted `## See Also` lists.** The canonical shape is one paragraph per pointer in the form `For more on X, see the [Y](Y.ipynb) page.` (or `notebook` instead of `page` for a tutorial target). A generic `## See Also` heading with bullets reads as reference-index genre and breaks tone with the surrounding prose. See the closing of `computing_graph_metrics.ipynb` for the canonical layout.
- **Exception — topic-specific closing sections are fine.** The rule against generic `## See Also` + bullets is about the reference-index genre, not about all section headings. When a closing pointer introduces a distinct conceptual topic worth exploring as its own subject, a topic-specific `## Heading` followed by prose is appropriate. The heading earns its place by naming a specific topic with its own depth, not by being a catch-all bucket. Examples: `setting_partition_variable.ipynb` and `setting_sorting_variables.ipynb` each close with `## Using Node Graph Metrics as <partition / sorting> Variables`.
- **Link to the single best "next step" notebook, not every subordinate reference.** When a topic has a focused walkthrough notebook and several supporting references (an API table, a per-method walkthrough, a kwarg reference), link to the walkthrough and rely on it to organize the rest. Inline enumerations like "see the [X Table](...), the [Y Method](...), and the [Z Walkthrough](...) for the supported keys, the post-hoc method, and the per-metric kwargs respectively" clutter the tutorial and duplicate organization the destination already provides.
- **Linking to a specific section of another notebook:** use the Sphinx-auto-generated anchor `[Section Name](other_notebook.ipynb#section-name-slug)`, where the slug is the lowercased heading with spaces replaced by hyphens. Example: `[Using a Computed Metric as a Partition Variable](computing_graph_metrics.ipynb#using-a-computed-metric-as-a-partition-variable)`. Plain `[Notebook Title](notebook.ipynb)` links remain correct for "go read the whole thing" pointers; use anchored links when the destination has multiple sections and the reader benefits from landing on a specific one.
