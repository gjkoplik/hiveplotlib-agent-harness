...
6. **Spec archive proposal.** Where a spec governs the plan, propose archiving it only when the spec's **own outcome statement is true** (plan-end qa's `Spec outcome: met` check is the usual evidence), never because a plan serving it shipped or archived: a spec may outlive every plan serving it, and a plan closing never closes the spec. Wiki specs move to `wiki/wiki/specs/archived/<topic>.md`, a suggestion the maintainer confirms and performs (agents never move spec files); harness-self specs stay flat. Where no spec governs the plan, there is nothing to propose and nothing to say.
7. Report.
...
- Propose archiving plans and specs; never move plan or spec files yourself. The human performs the move on confirmation.
