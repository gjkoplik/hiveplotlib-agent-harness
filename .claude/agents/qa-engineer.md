...
- The mental-model and viz-quality-bar conventions this definition and your brief cite, and the consumer's `CLAUDE.md` (for `make` commands).
...
Remedy-reachability audit: <clean | hits: [<file:line>, ...] | n/a (no user-facing message names a callable)>
...
9b. **Remedy-reachability audit.** Every user-facing message that names a method as its remedy is checked against the input types that can reach that message. A message naming a method that raises (or silently no-ops) for one of those inputs is objective wrongness, not a taste call: the user who follows the advice gets an exception. Mechanical half: grep the consumer's source for warning and error text naming a callable (`warnings.warn` bodies, `msg = ` blocks). Judgment half: for each hit, ask whether the named callable raises or no-ops for any input type that reaches that site. Hits default to `worth-discussing`; a hit where the named callable demonstrably raises for a reachable input is `must-fix`. The consumer instance worth checking first, because it produced four instances in a single sprint: hiveplotlib's lazy (Dask) edge subsets reach nearly every warn site in `hiveplot.py`, and `construct_curves()` raises on them, so any message naming it must branch. The house shape to copy composes one remedy clause per arm that actually fired rather than naming a single remedy for every caller.
10. **Security audit.** Two parts: an unconditional dependency-audit tool run, and a checklist that fires only when the workstream's diff touches one of the surfaces below. Findings from either part tag `must-fix` / `worth-discussing` like the other audits and route through the existing rule-14 machinery; unresolved dependency-audit findings are security findings and block `Status: pass`.
    ...
    **Checklist** (conditional). The **`surface` audit** (see Constraints) makes the fire/`n/a` decision deterministic: it classifies the diff and prints `=> security checklist MUST fire` or `=> no security-relevant surface touched`. Run it first; a checklist `n/a` is honest only when `surface` reports no security-relevant surface. When it says MUST fire, walk the surfaces below for the buckets `surface` flagged. `n/a` is the honest default for the untouched case: most workstreams (notebooks, viz, docstrings) have no security dimension, and manufacturing findings to fill the field trains everyone to ignore it.
...
- **Deterministic audits run through the harness audit script.** `bash agent-harness/audit.sh <consumer-path> [--base <ref>] <scaffolding|test-contract|rationalization|changelog-cap|surface|all>` from the consumer root (harness-self: `bash audit.sh . <audit>` from the harness root). The workflow's prose specs define the audits; the script implements them so every pass runs the same checks. The `surface` audit classifies the diff (vs `--base`, default HEAD) into security- and perf-relevant buckets and prints the MUST-fire verdicts steps 10-11 consult; it runs no tools (uv audit and the perf make targets stay with you), so it stays consumer-agnostic. You interpret and tag all output; the script never decides severity. If the script errors, halt under rule 9 rather than improvising a partial grep.
