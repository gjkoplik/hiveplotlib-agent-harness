...
- `.claude/agents/research-liaison.md`: wires the dev loop to the `hiveplotlib-llm-wiki`. Pre-task surfaces prior thinking; post-task updates the wiki.
...
- `.claude/agents/notebook-author.md`: creates or updates `examples/` notebooks. Delegates style to the consumer-repo tutorial/gallery skills. Applies polish-in-proportion-to-role.
- `.claude/agents/viz-critic.md`: read-only review of rendered figures. Confidence-tagged proposal list.
- `.claude/agents/verifier.md`: runs tests/lint/type/doc-build, audits replace-and-sweep, checks the Implementation log, auto-fixes deterministic issues, proposes taste-call concerns. Does not commit.
...
1. **Skeleton (complete):** mental-model skill, viz-quality-bar skill, plan template, Orchestrator, Code Engineer, Verifier.
