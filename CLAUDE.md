...
- **Workstream ships.** Invoke the **adversary in post-impl mode** on the shipped artifact, alongside the api-critic / viz-critic / editorial-critic post-impl passes. It attacks the diff blind first, then reads its own `### Adversary's challenge` and the orchestrator's disposition of it (plan-section-as-memory) and reconciles, writing `### Adversary post-impl`. The dispatch prompt states the read-order contract and its read mechanics: step 1 may read only the workstream block under review, its done-when criteria, `## Failure modes`, and `## Holdouts` (expected-survivor lists only); `### Adversary's challenge`, its disposition in `## Plan amendments`, and prior `### Adversary post-impl` history are off-limits until step 2; step 1 locates section offsets via Grep and uses ranged Reads, never a full-file Read of the plan. A post-impl `must-fix` / `worth-discussing` routes to orchestrator `amend-plan` like the other critics (the planning challenge does not; the maintainer fights that in the grill).
...
- **Mid-flight emergent work** (post-impl critic `must-fix` or `worth-discussing`, or any user ask that would change the workstream set per rule 14). Route to orchestrator in `amend-plan` mode before any other dispatch. Under auto-dispatch, a `worth-discussing` finding with no bearing on downstream not-yet-run workstreams batches to plan-end qa instead of routing immediately (see "Between workstreams" below). The orchestrator edits the plan; the dispatching session does not edit the plan directly.
